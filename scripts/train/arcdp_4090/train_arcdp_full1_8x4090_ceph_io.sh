...
PROFILE="${1:-}"
if [[ -z "${PROFILE}" ]]; then
    echo "Usage: $0 {b48_w2|b24_ga2_w2_pf1|b24_ga2_w4_pf1|b24_ga2_w4_pf1_safeckpt} [extra train.py arguments...]" >&2
    exit 2
fi
shift

PROFILE_PERCENT_CHECKPOINT_INTERVAL=0
PROFILE_PERCENT_CHECKPOINT_ROLLING_KEEP=5
PROFILE_PERCENT_CHECKPOINT_PERMANENT_INTERVAL=10
PROFILE_PERCENT_CHECKPOINT_VERIFY_LOADS=1
PROFILE_REQUIRE_COMPLETE_CHECKPOINT=0
...
case "${PROFILE}" in
    ...
    b24_ga2_w4_pf1)
        ...
        ;;
    b24_ga2_w4_pf1_safeckpt)
        PROFILE_BATCH_SIZE=24
        PROFILE_GRAD_ACCUM=2
        PROFILE_NUM_WORKERS=4
        PROFILE_PREFETCH_FACTOR=1
        PROFILE_RUN_NAME="arcdp_full1_b24_ga2_w4_pf1_safeckpt_8x4090_ceph"
        PROFILE_MASTER_PORT=12349
        PROFILE_PERCENT_CHECKPOINT_INTERVAL=1
        PROFILE_PERCENT_CHECKPOINT_ROLLING_KEEP=5
        PROFILE_PERCENT_CHECKPOINT_PERMANENT_INTERVAL=10
        PROFILE_PERCENT_CHECKPOINT_VERIFY_LOADS=1
        PROFILE_REQUIRE_COMPLETE_CHECKPOINT=1
        ;;
    *)
...
export BRIDGEDP_REQUIRE_COMPLETE_CHECKPOINT="${BRIDGEDP_REQUIRE_COMPLETE_CHECKPOINT:-${PROFILE_REQUIRE_COMPLETE_CHECKPOINT}}"
export BRIDGEDP_PERCENT_CHECKPOINT_INTERVAL="${BRIDGEDP_PERCENT_CHECKPOINT_INTERVAL:-${PROFILE_PERCENT_CHECKPOINT_INTERVAL}}"
export BRIDGEDP_PERCENT_CHECKPOINT_ROLLING_KEEP="${BRIDGEDP_PERCENT_CHECKPOINT_ROLLING_KEEP:-${PROFILE_PERCENT_CHECKPOINT_ROLLING_KEEP}}"
export BRIDGEDP_PERCENT_CHECKPOINT_PERMANENT_INTERVAL="${BRIDGEDP_PERCENT_CHECKPOINT_PERMANENT_INTERVAL:-${PROFILE_PERCENT_CHECKPOINT_PERMANENT_INTERVAL}}"
export BRIDGEDP_PERCENT_CHECKPOINT_VERIFY_LOADS="${BRIDGEDP_PERCENT_CHECKPOINT_VERIFY_LOADS:-${PROFILE_PERCENT_CHECKPOINT_VERIFY_LOADS}}"
...
echo "  checkpoint interval: ${BRIDGEDP_PERCENT_CHECKPOINT_INTERVAL}%"
echo "  rolling checkpoints kept: ${BRIDGEDP_PERCENT_CHECKPOINT_ROLLING_KEEP}"
echo "  permanent checkpoint interval: ${BRIDGEDP_PERCENT_CHECKPOINT_PERMANENT_INTERVAL}%"
echo "  verify checkpoint tensors: ${BRIDGEDP_PERCENT_CHECKPOINT_VERIFY_LOADS}"
echo "  resume only complete checkpoints: ${BRIDGEDP_REQUIRE_COMPLETE_CHECKPOINT}"
