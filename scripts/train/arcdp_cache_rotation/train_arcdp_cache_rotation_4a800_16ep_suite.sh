...
HDD_ROOT="/ssd"
NVME_ROOT="/nvme"
SWANLAB_PROJECT_VALUE="${SWANLAB_PROJECT:-ArcDP-16ep}"
...
ABLATION_EPOCHS="2"
SHARD_EPOCHS="10"
FULL_UNIFORM_CKPT_COUNT="8"
ABLATION_UNIFORM_CKPT_COUNT="2"
...
usage() {
    printf '%s\n' \
        "Usage:" \
        "  train_arcdp_cache_rotation_4a800_16ep_suite.sh [options]" \
        "" \
        "Runs full ArcDP for 16 equivalent epochs and all P0 ablations for 2 equivalent epochs on 4 x A800." \
        "The full run uses 8 uniform checkpoint archives, so the first archive is the 2-epoch full baseline." \
        "" \
        "Options:" \
        ...
        "  --swanlab-project NAME           Default: ArcDP-16ep" \
        "  --swanlab-workspace NAME" \
        "  --full-epochs N                  Default: 16" \
        "  --ablation-epochs N              Default: 2" \
        "  --shard-epochs N                 Default: 10" \
        "  --full-uniform-ckpts N           Default: 8" \
        "  --ablation-uniform-ckpts N       Default: 2" \
...
for epoch_value in "${FULL_EPOCHS}" "${ABLATION_EPOCHS}"; do
    case "${epoch_value}" in
        2|10|16|100|200|500|1000) ;;
        *) echo "suite epochs must be one of 2, 10, 16, 100, 200, 500, 1000; got: ${epoch_value}" >&2; exit 1 ;;
    esac
done
...
printf '%s\n' \
    "ArcDP 4 x A800 full-16ep / P0-2ep suite" \
    "  variants:       ${VARIANTS[*]}" \
