...
HDD_ROOT="/ssd"
NVME_ROOT="/nvme"
SWANLAB_PROJECT_VALUE="${SWANLAB_PROJECT:-ArcDP-4ep}"
...
usage() {
    printf '%s\n' \
        "Usage:" \
        "  train_arcdp_cache_rotation_4a800_4ep_suite.sh [options]" \
        "" \
        "Runs full ArcDP for 4 equivalent epochs and all P0 ablations for 1 equivalent epoch on 4 x A800." \
        "The full run uses 4 uniform checkpoint archives, so the first archive is the 1-epoch full baseline." \
        "" \
        "Options:" \
        ...
        "  --swanlab-project NAME           Default: ArcDP-4ep" \
        "  --swanlab-workspace NAME" \
        "  --full-epochs N                  Default: 4" \
        "  --ablation-epochs N              Default: 1" \
        "  --shard-epochs N                 Default: 10" \
        "  --full-uniform-ckpts N           Default: 4" \
...
for epoch_value in "${FULL_EPOCHS}" "${ABLATION_EPOCHS}"; do
    case "${epoch_value}" in
        ...
        *) echo "suite epochs must be one of 1, 2, 4, 6, 10, 16, 100, 200, 500, 1000; got: ${epoch_value}" >&2; exit 1 ;;
    esac
done
...
printf '%s\n' \
    "ArcDP 4 x A800 full-4ep / P0-1ep suite" \
    "  variants:       ${VARIANTS[*]}" \
