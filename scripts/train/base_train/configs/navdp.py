...
navdp_exp_cfg = ExpCfg(
    ...
    il=IlCfg(
        ...
        dataset_r2r_root_dir='data/vln_pe/raw_data/r2r',  # R2R 原始数据根目录。
        ...
        lerobot_features_dir='data/vln_pe/traj_data/r2r',  # LeRobot 特征路径（非 navdp 分支常用）。
        camera_name='pano_camera_0',  # 相机名称约定（用于特征选择/兼容字段）。
        report_to='tensorboard',  # 日志后端：wandb / tensorboard / none。
        dataset_navdp='data/datasets/navdp_dataset_lerobot.json',  # NavDP 预索引文件路径。
        root_dir='data/datasets/InternData-N1/vln_n1/traj_data',  # NavDP 轨迹数据根目录。
        image_size=224,  # 输入图像分辨率（RGB/Depth 统一到该尺寸）。
        scene_scale=1.0,  # 场景采样比例，1.0 表示不过滤场景。
        preload=False,  # 是否直接从 dataset_navdp 读取预构建索引。
